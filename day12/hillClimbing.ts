...
export function hillClimbing(file: string): number {
  ...
  const start: number = nodes.findIndex((el) => el.height === "S");
  const end: number = nodes.findIndex((el) => el.height === "E");
  nodes[start].height = "a";
  nodes[end].height = "z";
